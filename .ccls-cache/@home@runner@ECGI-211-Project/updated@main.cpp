...
//bam&guy:we(mb)are gonna do the stacks here
       /* cout << "Branch is full. Unable to add more Employee."; */
        cout<< "Branch is full. Do you want to replace the last employee? (y/n)";
        cin >> opt;
        if(opt == 'y'){
          //pop top
          comp->popLast(num);
          cout << "An employee is removed from the branch "<<endl;
          cout<< comp->getBranch(num).getLo() << ".";
          //insert new input
          comp->addEmployee(num, name);
            cout << "Employee " << name << " is added to branch " <<endl;
             cout<< comp->getBranch(num).getLo() << ".";
        }//end if(2)
      }//end else
